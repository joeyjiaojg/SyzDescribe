...
size_t funcHash(Function *F, bool withName) {
    ...
    //FTy->print(rso);
    //output = rso.str();
    output = get_real_function_type(F->getFunctionType());

    yhao_log(-1, "MLTA output                : " + output);
    yhao_log(-1, "MLTA get_real_function_type: " + get_real_function_type(F->getFunctionType()));

    if (withName) {
        //output += F->getName();
        output += get_real_function_name(F);
    }
#ifdef HASH_SOURCE_INFO
    }
#endif
...
size_t callHash(CallInst *CI) {
    ...
    Function *CF = get_target_function(cs.getCalledOperand());
    ...
    if (CF)
        ...
        //FTy->print(rso);
        //strip_str = rso.str();
        strip_str = get_real_function_type(cs.getFunctionType());
...
size_t typeHash(Type *Ty) {
    ...
    if (STy == NULL)
        ...
        //Struct number
        if (STy->hasName()) {
            string STyname = get_real_structure_name(STy->getName().str());
